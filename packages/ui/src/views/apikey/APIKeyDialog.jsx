...
const APIKeyDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    const addNewKey = async () => {
        try {
            ...
            if (createResp.data) {
                enqueueSnackbar({
                    message: t('apikey.newApiKeyAdded'),
                    options: {
            ...
            enqueueSnackbar({
                message: `${t('apikey.failedToAddApiKey')} ${
    ...
    const saveKey = async () => {
        try {
            ...
            if (setError) setError(error)
            enqueueSnackbar({
                message: `${t('apikey.failedToSaveApiKey')} ${
                    typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                }`,
                options: {
                    key: new Date().getTime() + Math.random(),
        ...
        >
            ...
            <DialogContent>
                {dialogProps.type === 'EDIT' && (
                    <Box sx={{ p: 2 }}>
                        <Typography variant='overline'>{t('apikey.apiKey')}</Typography>
                ...
                <Box sx={{ p: 2 }}>
                    <Stack sx={{ position: 'relative' }} direction='row'>
                        <Typography variant='overline'>{t('apikey.keyName')}</Typography>
                    </Stack>
                    <OutlinedInput
                        id='keyName'
                        type='string'
                        fullWidth
                        placeholder={t('apikey.keyName')}
                        value={keyName}
                        name='keyName'
